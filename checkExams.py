...
class Enrolled:
    ...
    def showStudents(kind="not eligible", exams=False):
        # print('.....', len(Enrolled.students));input()
        if not Enrolled.students: return ""
    ...
    def load(dir, progressFile):
        ...
        elif os.path.isfile(os.path.join(dir, progressFile)):
            Enrolled.progressFile = os.path.join(dir, progressFile)
        else:
            return'δεν βρέθηκαν αρχεία από το progress στον ορισμένο φάκελο'
